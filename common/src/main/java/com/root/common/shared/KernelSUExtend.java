...
public class KernelSUExtend {
    // source /data/adb/util_functions.sh

    public static String MAGISK_PATH = "/sbin/.core/img/scene_systemless/";
    private static String MAGISK_PATH_11528 = "/data/adb/modules"; //  "/sbin/.magisk/modules";
    ...
    private static boolean spaceValidation(long require) {
        // magisk 11528开始，不用镜像了，理论上空间无限
        if (MagiskVersion >= 11528 || MAGISK_PATH.startsWith("/data")) {
            return true;
        }
    ...
    public static boolean createFileReplaceModule(String orginPath, String newfile, String moduleName, String moduleTitle) {
        if (KernelSUExtend.magiskSupported() && RootFile.INSTANCE.itemExists(newfile)) {
            String moduleProp = 
                    "id=rootes\\n" +
                    "name=玩机百宝箱\\n" +
                    "version=v1.0.0\\n" +
                    "versionCode=100\\n" +
                    "author=玩机百宝箱\\n" +
                    "description=开机自动启动sh\n" +
                    "minMagisk=17000\\n";

            if (MagiskVersion < 11328) {
                ...
                String output = MAGISK_PATH_11528 + "/" + moduleName + ((orginPath.startsWith("/vendor") || orginPath.startsWith("/product")) ? ("/system" + orginPath) : orginPath);
                String dir = new File(output).getParent();
                KeepShellPublic.INSTANCE.doCmdSync(
                        "mkdir -p \"" + dir + "\"\n" +
                                "echo \"" + moduleProp + "\" > \"" + MAGISK_PATH_11528 + "/" + moduleName + "/module.prop\"\n" +
                                "cp -pdrf \"" + newfile + "\" \"" + output + "\"\n" +
                                "chmod -R 777 \"" + output + "\"");
            }
    ...
    public static void magiskModuleInstall(Context context) {
        String moduleProp = "id=scene_systemless\n" +
                ...
                "versionCode=1\n" +
                "author=玩机百宝箱\n" +
                "description=玩机百宝箱拓展模块，从而在不修改系统文件的情况下，更改一些参数\n" +
                "minMagisk=17000\n";
    ...
    public static boolean magiskSupported() {
        if (supported == -1 || MagiskVersion < 1) {
            String magiskVersion = KeepShellPublic.INSTANCE.doCmdSync("su -V");
            if (!magiskVersion.equals("error")) {
                try {
                    MagiskVersion = Integer.parseInt(magiskVersion) / 1000;
                    supported = MagiskVersion >= 11328 ? 1 : 0;

                    if (supported == 1) {
                        if (MagiskVersion >= 11528) {
                            MAGISK_PATH = MAGISK_PATH_11528 + "/" + MAGISK_MODULE_NAME + "/";
