...
import com.root.system.AccessibilityScenceMode
import com.root.system.popup.FloatMonitorMini
import com.root.system.popup.FloatScreenRotation
...
class SceneMode private constructor(private val context: AccessibilityScenceMode, private var store: SceneConfigStore) {
    ...
    private fun backupLocationModeState() {
        if (locationMode == "none") {
            locationMode = Settings.Secure.getString(contentResolver, Settings.Secure.LOCATION_MODE)
        }
    }
    ...
    // 先使用 currentGPSOn() 方法检查
    if (!currentGPSOn()) {
        // 再使用 Settings.Secure.getString 来进一步检查
        val mode = Settings.Secure.getString(contentResolver, Settings.Secure.LOCATION_MODE)
        if (mode == null || !mode.contains("gps")) {
