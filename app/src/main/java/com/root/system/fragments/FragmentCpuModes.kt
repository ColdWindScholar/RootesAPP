...
import com.root.library.shell.ThermalDisguise
...
class FragmentCpuModes : Fragment() {
    ...
    private fun chooseConfigSource() {
        ...
        view.findViewById<View>(R.id.source_download1).setOnClickListener {
...
yinwanxi()
        ...
        view.findViewById<View>(R.id.source_download2).setOnClickListener {
...
shadow3aaa()
    ...
    private fun bindMode1(button: View, mode: String) {
    ...
    private fun installLocalConfig(path: String) {
        ...
        if (file.exists()) {
            ...
            if (lines == null) {
                Toast.makeText(context, "玩机百宝箱无法读取此文件！", Toast.LENGTH_LONG).show()
                return
            }
