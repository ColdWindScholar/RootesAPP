            ...
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                scope.launch(Dispatchers.IO) {
                    try {
                        ...
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@ActivityModules, "无法连接服务器，请检查网络设置: ${ex.message}", Toast.LENGTH_SHORT).show()
                        }
                    } finally {
                        view.post {
                            view.isEnabled = true
        ...
        scope.launch {
            try {
                ...
                if (modules.isNotEmpty()) {
                    ...
                    Toast.makeText(this@ActivityModules, "无法找到模块", Toast.LENGTH_SHORT).show()
                }
            } catch (ex: Exception) {
                Log.e("ActivityModules", "Error on item click", ex)
                Toast.makeText(this@ActivityModules, "下载模块失败: ${ex.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    cursor?.use {
        if (it.moveToFirst()) {
            ...
            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                ...
                val newFile = File(file.parent, file.name.replace(".txt", ".zip")) // 将 .txt 替换为 .zip
