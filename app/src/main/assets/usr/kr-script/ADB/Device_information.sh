...
echo "查看设备连接状态"
$binariesPath/libfastboot.so devices | sed 's/fastboot/　&/g'
p
#echo "BL状态：`fastboot getvar unlocked 2>&1 | egrep -q 'unlocked.*yes' && echo "已解锁" || echo "未解锁"`"
echo "查看设备序列号"
$binariesPath/libfastboot.so getvar serialno
[[ $? -eq 126 ]] && exit 126
echo "设备产品代号"
$binariesPath/libfastboot.so getvar product
echo "设备储存类型"
$binariesPath/libfastboot.so getvar variant
echo "检查是否允许解锁，仅供参考"
echo "显示：get_unlock_ability: 1，说明允许解锁"
echo "显示：get_unlock_ability: 0，说明不允许解锁"
$binariesPath/libfastboot.so flashing get_unlock_ability
...
$binariesPath/libfastboot.so oem device-info
p
echo ②
echo "理论上显示：unlocked: yes，说明已解锁"
echo "理论上显示：unlocked: no，说明未解锁"
$binariesPath/libfastboot.so getvar unlocked
p
echo ③
echo "查看MTK BL解锁状态"
echo "理论上显示：lks = 0，说明已解锁"
echo "理论上显示：lks = 1，说明未解锁"
$binariesPath/libfastboot.so oem lks
p
echo ④
echo "理论上显示：Lock State: UNLOCKED，说明已解锁"
echo "理论上显示：Lock State: LOCKED，说明未解锁"
$binariesPath/libfastboot.so oem get-bootinfo
if [[ $ShowAll -eq 1 ]]; then
echo
p
echo "已选择查看设备全部信息"
$binariesPath/libfastboot.so getvar all
fi
