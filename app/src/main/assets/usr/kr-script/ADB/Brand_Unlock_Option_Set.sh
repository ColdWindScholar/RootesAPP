...
if [[ $Brand = Lenovo ]]; then
    ...
    echo "如果提示OKAY代表解锁成功"
    $binariesPath/libfastboot.so flash unlock $PeiZhi_File/unlock_bootloader.img
    $binariesPath/libfastboot.so oem unlock-go
    ...
    $binariesPath/libfastboot.so flashing unlock
elif [[ $Brand = Google_Pixel ]]; then
    echo "已选择一键解锁google_pixel系列bl锁"
    echo "如果提示okay代表解锁成功"
    $binariesPath/libfastboot.so flashing unlock
    $binariesPath/libfastboot.so flashing unlock_critical
fi
